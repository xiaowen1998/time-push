...
def caihongpi():
    if (Whether_caihongpi!=False):
        try:
            conn = http.client.HTTPSConnection('http://api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'8c2c363c02209967df63657cf26bebf7'})
...
def lucky():
    if ( Whether_lucky!=False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'8c2c363c02209967df63657cf26bebf7','astro':"taurus"})
            headers = {'Content-type':'application/x-www-form-urlencoded'}
...
def tip():
    if (Whether_tip!=False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'8c2c363c02209967df63657cf26bebf7','city':city})
            headers = {'Content-type':'application/x-www-form-urlencoded'}
